...
from .utils import ensure_dir, try_call, save_as, focus, shot
...
# ─── ШАГ 1  Форматирование таблицы ───────────────────
@safe_step("Лаба2_01.mpp")
def s01(app):
    try_call(app.ViewApply, "Гант", "Диаграмма Ганта", "Gantt Chart")
    # показать таблицу «Ввод»
    try_call(app.TableApply, "Ввод", "Entry")
    # удалить столбец ID
    if try_call(app.SelectColumn, "ID", "ИД", "№"): app.EditDelete()
    # добавить «Критическая задача»
    try_call(app.InsertColumn, "Критическая задача", "Critical")
    # заменить на «Затраты»
    if try_call(app.SelectColumn, "Критическая задача", "Critical"):
        app.InsertColumn("Затраты", "Cost")
        app.EditDelete()                       # убрать старый столбец
    # стили текста
    try:
...
def s05(app):
    ...
    try_call(app.FilterEdit, "NextMon", True, True,
             ...
             False)
    try_call(app.FilterApply, "Dur15");  try_call(app.FilterApply, "NextMon")

# ─── ШАГ 6  Предопределённый фильтр «Суммарные» ──────
...
# ─── ШАГ 8  Группировка «Вехи/не вехи» ───────────────
...
def s09(app):
    if try_call(app.GroupEdit, "CritDur", True, True,
                "Critical", False, False,      # первая группа, order desc
                "Duration", True, True,        # вторая  группа, order asc
                False):
        try_call(app.GroupApply, "CritDur")

# ─── ШАГ 10 Time-группировка по неделям ───────────────
@safe_step("Лаба2_10.mpp")
def s10(app):
    try_call(app.GroupEdit, "ByWeek", True, True,
             "Duration", True, True, Interval=1, IntervalUnit=3)
    try_call(app.GroupApply, "ByWeek")

# ─── ШАГ 11 Формат одного Normal-bar ──────────────────
@safe_step("Лаба2_11.mpp")
def s11(app):
    try_call(app.BarStyleEdit, "Normal", "Normal", 1, "Start", "Finish", 1, 1)

# ─── ШАГ 12 Формат всех Normal-bar ─────────────────────
@safe_step("Лаба2_12.mpp")
def s12(app):  try_call(app.BarStyleEdit, "Normal", "Normal", 1, "Start", "Finish", 1, 1)

# ─── ШАГ 13 Критические красным ───────────────────────
@safe_step("Лаба2_13.mpp")
def s13(app):  try_call(app.BarStyleEdit, "Critical", "Critical", 1, "Start", "Finish", 1, 2)

# ─── ШАГ 14 Шкала: верх-tier = месяц, н/р жёлтый ──────
@safe_step("Лаба2_14.mpp")
def s14(app):
    try:
        app.TimescaleTopTier(3)    # 3 — Months
        # цвет нераб. времени через CalendarExceptions недоступен глобально — пропустим
    except Exception: pass
...
def run():
    ...
    if not PROJECT.exists():
        print(f"⚠️  {PROJECT} не найден – создаю новый проект-заглушку")
        app.FileNew(); save_as(app, str(PROJECT))
    else:
        app.FileOpen(str(PROJECT))

    # сворачиваем/разворачиваем, чтобы UI был виден
    try:
    ...
    for idx, fn in enumerate(steps, 1):
        fn(app, idx, fn.__name__); time.sleep(PAUSE)

    print(f"✅ Lab 2 completed for variant {variant}")
